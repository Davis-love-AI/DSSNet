...
if __name__ == '__main__':
    for i in range(5):
        ...
        for images in tqdm(test_loader, total=len(test_loader)):
            ...
            pred =  pred[:, pad_left:fine_size + pad_left, pad_left:fine_size + pad_left]
        ...
        pred_null = np.concatenate(pred_null).reshape(-1, 202, 202)
        pred_flip = np.concatenate(pred_flip).reshape(-1, 202, 202)
        print(pred_null.shape)
        print(pred_flip.shape)
