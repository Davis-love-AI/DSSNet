    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if self.fine_size != image.shape[0]:
                image,_ = do_resize2(image, mask, self.fine_size, self.fine_size)
                _, mask = do_resize2(image, mask, 128, 128)
            if self.pad_left != 0:
                image, _ = do_center_pad2(image, mask, self.pad_left, self.pad_right)
